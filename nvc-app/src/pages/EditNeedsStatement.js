...
function EditNeedsStatement (props) {
  ...
  const { title, description, feeling, need } = thisConflict;
  ...
  const activePageDisplay =()  => {
    if (activePage === 0){
      return <NeedsStatementForm formData = {formData} setFormData = {setFormData}/>
    } else {
      return <NeedsStatementFinalEdit formData = {formData} setFormData = {setFormData}/>;
    }
  }
